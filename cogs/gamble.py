...
class gamble(commands.Cog):
        ...
        try:
            ...
            if winnings > bet:
                embed = discord.Embed(
                    ...
                    description=f"You Won: {winnings - bet} Pinecones\n\nYou have {guild_data['coins'][str(ctx.author.id)]} Pinecones",
                    color=0x06C258,
                )
                embed.set_author(
                ...
                embed = discord.Embed(
                    title="OOPSIE",
                    description=f"You LOST: {bet - winnings} Pinecones\n\nYou have {guild_data['coins'][str(ctx.author.id)]} Pinecones",
    ...
    async def mb(self, ctx: commands.Context):
        ...
        if str(ctx.author.id) not in guild_data["coins"]:
            ...
            return await ctx.send(
                "You do not have enough coins. A mystery box is 1000 Pinecones."
            )
        mysterybox = random.randint(-200, 2000)

        guild_data["coins"][str(ctx.author.id)] += mysterybox - mysteryboxprice
        if guild_data["coins"][str(ctx.author.id)] < 0:  # NEGATIVE CHECK
            guild_data["coins"][str(ctx.author.id)] = 0
        await Database.set_guild(ctx.guild.id, guild_data)

        if mysterybox < mysteryboxprice:
            embed = discord.Embed(
                title="OOPSIE",
                description=f"You Won: {mysterybox} Pinecones\n\nYou have {guild_data['coins'][str(ctx.author.id)]} Pinecones",
                color=discord.Color.red(),
            )
            embed.set_author(
                name=ctx.author.display_name,
    ...
    @commands.command()
    @commands.max_concurrency(1, per=commands.BucketType.member)
    async def sb(self, ctx: commands.Context):
        """10000 Pinecones. You may win money or you may loose money"""
        guild_data = await Database.get_guild(ctx.guild.id)
        mysteryboxprice = 10000
        if str(ctx.author.id) not in guild_data["coins"]:
            return await ctx.send("Please init your account first.")
        elif guild_data["coins"][str(ctx.author.id)] < mysteryboxprice:
            return await ctx.send(
                "You do not have enough coins. A super box is 10000 Pinecones."
            )
        mysterybox = random.randint(3000, 15000)
    ...
    async def sub(self, ctx: commands.Context):
        """100000 Pinecones. You may win money or you may loose money"""
        guild_data = await Database.get_guild(ctx.guild.id)
        mysteryboxprice = 100000
        if str(ctx.author.id) not in guild_data["coins"]:
            return await ctx.send("Please init your account first.")
        elif guild_data["coins"][str(ctx.author.id)] < mysteryboxprice:
            return await ctx.send(
                "You do not have enough coins. A super ultra box is 100000 Pinecones."
            )
        mysterybox = random.randint(50000, 150000)
