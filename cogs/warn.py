...
class warning(commands.Cog):
    ...
    async def warn(self, ctx, user: discord.User, *, reason):
        ...
        guild_data["warns"][str(user.id)].append(
            {"author_id": ctx.author.id, "reason": reason}
        )

        await Database.set_guild(ctx.guild.id, guild_data)
        embed = discord.Embed(
            description=f"{user} Warned for: {reason}", color=0x06C258
        )
        embed.set_author(
            name=user.name + "#" + user.discriminator, icon_url=user.avatar.url
        )
    ...
    async def delwarn(self, ctx, user: discord.User, warnId: int):
        ...
        try:
            ...
            return await ctx.send(
                "Invalid warning ID given, please check the ID and try again."
            )
        else:
            await Database.set_guild(ctx.guild.id, guild_data)
    ...
    async def warns(self, ctx, user: discord.User):
        ...
        for userwarndata in userwarnlist:
            if numWarns != 25:
                ...
                embed.add_field(
                    name=str(numWarns) + ". " + userwarndata["reason"],
                    value=f"Warned by: {warner}",
                    inline=False,
                )
        await ctx.send(embed=embed)
